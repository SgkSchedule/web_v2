<template>
  <div class='grid w-full gap-1.5' v-bind:class="'gap-rows-' + this.rasp.length">
    <div v-for="(item) in this.rasp" :key="item.id" class="grid w-full gap-x-2 sm:gap-x-1.5 grid-cols-10">
      <row-item class="sm:col-span-1" v-bind:class="[activeTab == 'user' || activeTab == 'cabinet' ? 'col-span-1' : 'col-span-2']" v-if="item.isHeader == undefined">
        <span>{{item.num}}</span>
      </row-item>
      <row-item class="sm:col-span-1" v-bind:class="[activeTab == 'user' || activeTab == 'cabinet' ? 'col-span-1' : 'col-span-2']" v-if="item.isHeader == undefined">
      ...
      <row-item class="!justify-start" v-if="item.isHeader == undefined" v-bind:class="[activeTab == 'user' || activeTab == 'cabinet' ? 'col-span-5' : ['col-span-5', 'sm:col-span-7']]">
        <div>
